import { DIPLOMACY_STATE_EVENT_POOLS } from '../../data/diplomacyEvents.js';
...
export function resolveMonthlyDiplomacyEvents({ factions, playerCities, nextCities }) {
  ...
  Object.values(factions)
    ...
    .forEach((faction) => {
      const state = getDiplomacyState(faction);
      if (!state) {
        return;
      }
      ...
      if (Math.random() * 100 >= getDiplomacyEventChance(faction, state)) {
        return;
      }
